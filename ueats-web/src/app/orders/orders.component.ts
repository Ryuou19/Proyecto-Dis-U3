  ...
  async loadOrdersWithProducts(accountId: number) {
    try {
      ...
      const stores = await firstValueFrom(
        this.http.get<any[]>(`${this.apiUrl}:8082/store?city=${userCity}`)
      );
  
      const orders = await firstValueFrom(
        this.http.get<Order[]>(`${this.apiUrl}:8083/order?accountId=${accountId}`)
      );
  
      const productsPromises = stores.map(store => 
        firstValueFrom(this.http.get<Product[]>(`${this.apiUrl}:8082/product?storeId=${store.id}`))
      );
        ...
        try {
          for (const store of stores) {
            const hasRating = await firstValueFrom(
              this.http.get<boolean>(`${this.apiUrl}:8084/rating/check?orderId=${order.id}&storeId=${store.id}`)
            );
            if (hasRating) {
    ...
    try {
      ...
      for (const store of stores) {
        const products = await firstValueFrom(
          this.http.get<Product[]>(`${this.apiUrl}:8082/product?storeId=${store.id}`)
        );
      ...
      await firstValueFrom(
        this.http.post(`${this.apiUrl}:8084/rating`, ratingData)
      );
